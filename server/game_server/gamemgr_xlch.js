...
function shuyuanfen(shuyuan, num_shuyuan) {
    var ans = 0;
    if (shuyuan.indexOf(20) > -1 && shuyuan.indexOf(23) > -1 && shuyuan.indexOf(24) > -1 &&
        shuyuan.indexOf(3) > -1 && shuyuan.indexOf(2) > -1 && shuyuan.indexOf(1) > -1) {
        return "6书院大圆满";
    }
    if (shuyuan.indexOf(20) > -1 && shuyuan.indexOf(23) > -1 && shuyuan.indexOf(24) > -1) {
        return "湖畔3书院";
    }
    if (shuyuan.indexOf(1) > -1 && shuyuan.indexOf(2) > -1 && shuyuan.indexOf(3) > -1) {
        return "荔园3书院";
    }
    if (shuyuan.length >= 12) {
        return "书院大集合";
    }
    if (shuyuan.length >= 8) {
        return "资深书院收集者";
    }
    return shuyuan.length.toString() + "书院收集";
}

function chifanfen(chifan) {
    if (chifan >= 10) {
        return "大胃王";
    }
    if (chifan >= 6) {
...
function cal_gpa(gameSeatData) {
    totalpai = [];
    for (var i = 0; i < gameSeatData.angangs.length; i++) {
        for (var j = 0; j < 4; j++) {
            totalpai.push(gameSeatData.angangs[i]);
        }
    }
    for (var i = 0; i < gameSeatData.diangangs.length; i++) {
        for (var j = 0; j < 4; j++) {
            totalpai.push(gameSeatData.diangangs[i]);
        }
    }
    for (var i = 0; i < gameSeatData.wangangs.length; i++) {
        for (var j = 0; j < 4; j++) {
            totalpai.push(gameSeatData.wangangs[i]);
        }
    }
    for (var i = 0; i < gameSeatData.pengs.length; i++) {
        for (var j = 0; j < 3; j++) {
            totalpai.push(gameSeatData.pengs[i]);
        }
    }
    for (var i = 0; i < gameSeatData.holds.length; i++) {
        totalpai.push(gameSeatData.holds[i]);
    }
        ...
        var shenghuo = 0;
        var chifan = 0;
        for (var i = 0; i < totalpai.length; i++) {
            var pai = totalpai[i];
            var temp = getMJType(pai);
            if (temp == 0) {
                liyuan++;
            } else if (temp == 1) {
                qita++;
            ...
            if (pai == 3 || pai == 4 || pai == 5 || pai == 6 || pai == 7 || pai == 11 || pai == 9 || pai == 10) {
                xuexi++;
            }
            if (pai == 22 || pai == 19 || pai == 18 || pai == 25 || pai == 17 || pai == 16) {
                yundong++;
            }
            if (pai == 1 || pai == 2 || pai == 3 || pai == 20 || pai == 23 || pai == 24 || pai == 21 || i == 26) {
                shenghuo++;
                if (pai != 21) {
                    shuyuan.push(pai);
                }
            }
            if (pai == 12 || pai == 13 || pai == 14 || pai == 0) {
                chifan++;
            }
            
        }
        if (shuxingfen(liyuan) != 0) {
            gpa.push(shuxingfen(liyuan) + "荔园");
        }
        if (shuxingfen(hupan) != 0) {
            gpa.push(shuxingfen(hupan) + "湖畔");
        }
        if (shuxingfen(qita) != 0) {
            gpa.push(shuxingfen(qita) + "其他");
        }
        gpa.push(xuexifen(xuexi));
            gpa.push(tiyufen(yundong));
            gpa.push(shuyuanfen(shuyuan));
            gpa.push(shenghuofen(shenghuo));
            gpa.push(chifanfen(chifan));
            console.log("*********");
            console.log(liyuan);
        console.log(hupan);
        console.log(qita);
...
function doGameOver(game, userId, forceEnd) {
    ...
    if (game != null) {
        ...
        for (var i = 0; i < roomInfo.seats.length; ++i) {
            var rs = roomInfo.seats[i];
            var sd = game.gameSeats[i];
            db.cost_gems(sd.userId,sd.score);
...
exports.peng = function(userId) {
    ...
    //和的了，就不要再来了
    if (seatData.hued) {
        console.log('you have already hued.');
        return;
    }
    //如果有人可以胡牌，则需要等待
    var i = game.turn;
